...
def main(page: ft.Page):
    ...
    # --- NUEVA FUNCIÓN DE CONFIGURACIÓN CONECTADA ---
    def mostrar_configuracion(e):
        close_dialog() # Cierra el perfil antes de abrir configuración
        ...
        def warning_delete(e_btn):
            show_snack("Advertencia: Esta función borrará todos los datos.", ft.colors.RED_600)

        delete_btn = ft.ElevatedButton("Borrar todos los datos", color=ft.colors.WHITE, bgcolor=ft.colors.RED_600, on_click=warning_delete)

        content = ft.Column([theme_switch, nota_minima_input, delete_btn], tight=True)

        def volver_perfil(e_btn):
            close_dialog()
            mostrar_perfil(e_btn)

        show_dialog("Configuración", content, [
    ...
    page.bottom_appbar = ft.BottomAppBar(
        ...
        content=ft.Row([
            ft.Container(content=ft.Column([ft.Icon(ft.icons.HOME, color="#A8E6CF"), ft.Text("Inicio", color="#A8E6CF", size=10)], alignment=ft.MainAxisAlignment.CENTER, spacing=0), ink=True, border_radius=10, padding=5, on_click=lambda e: refresh_ui(vista_actual="inicio")),
            ft.Container(content=ft.Column([ft.Icon(ft.icons.MENU_BOOK, color="#64748B"), ft.Text("Materias", color="#64748B", size=10)], alignment=ft.MainAxisAlignment.CENTER, spacing=0), ink=True, border_radius=10, padding=5, on_click=lambda e: refresh_ui(vista_actual="materias")),
            ...
            ft.Container(content=ft.Column([ft.Icon(ft.icons.ACCOUNT_CIRCLE, color="#64748B"), ft.Text("Cuenta", color="#64748B", size=10)], alignment=ft.MainAxisAlignment.CENTER, spacing=0), ink=True, border_radius=10, padding=5, on_click=mostrar_perfil),
